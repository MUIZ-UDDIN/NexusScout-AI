    ...
    try:
        async with httpx.AsyncClient(timeout=10, follow_redirects=True) as client:
            ...
            if r.status_code != 200:
                return []
            found = list(set(re.findall(EMAIL_REGEX, r.text, re.I)))
...
async def _enrich_one(lead, page) -> None:
    ...
    try:
        async with AsyncSessionLocal() as session:
            ...
            for idx, lead in enumerate(leads):
                ...
                try:
                    await asyncio.wait_for(_enrich_one(lead, page), timeout=120)
                except asyncio.TimeoutError:
                    lead.status = "enriched" if (lead.phone or lead.address) else "failed"
                    print(f"[!] {lead.name} enrichment timed out, marking {lead.status}")
